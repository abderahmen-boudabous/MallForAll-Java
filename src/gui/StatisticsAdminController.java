...
public class StatisticsAdminController implements Initializable {
    ...
    try {
        ...
        List<Rec> rec = rs.recuperer();

        // Regrouper les réclamations par type
        Map<String, Long> reclamationsParType_reclamation = rec.stream()
                .collect(Collectors.groupingBy(Rec::getSujet, Collectors.counting()));
    ...
    @FXML
    private void gb(ActionEvent event) {
           FXMLLoader loader = new FXMLLoader(getClass().getResource("menur3.fxml"));
                        try {
                            Parent root = loader.load();
